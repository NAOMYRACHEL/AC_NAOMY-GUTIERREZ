...
#Tenemos que creear una funcion que tome como parametros otra funcion y su deribaja donde nos den como salida una aproximacion de raiz.

function newton(f, f′, x0, n)            #Definimos la funcion y sus parametros
		sol = x0 - (f(x0)/f′(x0))         #Colocamos lo que queremos que nos regrese como solucion que es la 
        ...
        print(sol)              	     #le pedimos que nos mande los resultados.
end
...
#colocamos una funcion y su derivada, y cambiamos de f a h por que si volvemos a poner f sale un error de que se ha llamado mucho f.
...
function newtonRecursiva(f :: Function, f′, x1::Number, m) #Colocamos la funcion con sus parametros
   sol_2= x1 - (f(x1)/f′(x1))     #colocamos la definicion de la aproximacion de la raiz.

             # nos pide que sea recursiva, como hemos visto en las clases, es correcto usar condiciones de if y de else.
	if m == 0 
		println(sol_2)
...
#colocamos una funcion y su derivada, con la caracteristica de buscar un funcion periodica.
...
function newtonDistancia(f, f′, x2, ε)  #Nuevamente colocamos una funcion con sus parametros.
	
	#nos pide que al momento en que se realicen distintas iteraciones y que cuando dos se aproximen sien ε menor a cero se marque un mensaje de error. (esto ultimo lo ponemos en la siguiente condicion if)

	if ε ≤ 0               #iniciamos un ciclo if esto para poder colocar el valor de ε.
		print("Error")

		#creamos una opcion cuando la aproximacion sea mayor a ε, siendo x2+ε, devido que queremos una aproximacion positiva.

	else ε > 0 
	 sol_3 =  x2+ε- ((f(x2+ε)/f′(x2+ε)))     #ahora vamos a conseguir que cuando ε sea mas grande se se imprima nustra funcion de aproximacion de raiz.
...
function derivadaNumerica(fd :: Function, x3:: Real , h:: Number) #definimos la funcion con los parametros que nos piden.
	der_1= (h*(fd(x3+1/h)-fd(x3)))    #Colocamos que queremos que nos regrese que en este caso es la 
...
derivadaNumerica(p, 10, 10*100000)
...
function newtonDN(f, x4, h , n :: Number) #Por como nos indica el problema nuestros parametros de entrada solo sera 
	    #para implementar el metodo de newton opteniendo una raiz, necesitamos generar la derivada de la funcion dentro del codigo.
...
begin
plot((x5,ffin), label= "cos" ) #ordenamos que se imprima la grafica con la funcion y la aproximacion.
	#colocamos un scatter para que nos grafuiqye los valores de la aprocimacion.
scatter!((x5,ffin), xlims=(-15.7,15.7), ylims=(-1.0,1.0), xlabel = "xf",  color = ["purple" colorant"#00ff00"], ylabel = "fd2", label= "aprox")
	#el coseno en sus limites vertical tiene el limite de uno por el circulo unitario, en las del eje x no tiene por lo qu solo lo limitamos para que se logre visualisar la funcion coseno.
end

#aqui se supone que se deberia modificar al desplazar los datos pero no cambia, por que fije el dato pero no supe como quitarlo, por que cuando lo intentaba me decia que no podia hacer eso. 
