...
# ╔═╡ 55a6c0c1-5734-41b0-a557-05a56c140426
@bind d Slider(0.05:0.1:2, default=0.1)

# ╔═╡ 4fe1aa65-6b5c-4e97-8d27-8e812aad5c04
d

# ╔═╡ 7497b549-45a8-4c72-aca9-981622fd54a1
begin
	plot(sin, -π:d:π, title ="Funciones trigonometricas",  ylabel ="sin(x)", color ="red", label =  "sen(x)", marker = true)
	plot!(cos, -π:d:π, xlabel = "x ", ylabel =   "f(x)"  , color = "blue", label = "cos(x)", marker = true)
end
...
#Definimos las variables.
	...
	Al = 10     #Aqui solo como comentario me decia que A y h como se pide en el ejercicio pero decia que
	r = 4       #ya lo habia ocupado mucho por lo que cambie de nombre.
	θ = π/4
	
#Debido a que hablamos de un tiro paramolico, tenemos que definir los componentes del vector velocidad.

	Vx = r*cos(θ)
	Vy = r*sin(θ)

# ya quetenemos devinido los componentes del vector velocidad, los podemos usar para definir el vector de posicion.

	M(x)   = ((Vx)*x, Al + (Vy)*x  -9.81*x^2/2)       
	Mx(x) = ((Vx)*x)                                
	My(x) = (Al +(Vy)*x  -9.81*x^2/2)             

			maxn = 	Vx*((2*Vy/9.81+((2*Vy/9.81)^2 + 8*A/9.81)^(1/2))/2) #Como tendremos una grafica tenemos 
	may  =  Al + (Vy)*(Vy/9.81) - 9.81*(Vy/9.81)^2/2                     # delimitar los limites de este 
	                                                                      # vestor posicion que establecimos.

	anima = @animate for t in 0:0.1:4 # Por cada t ∈ 0:0.1:4,
	plot(M.( range(-(5),0, step = 0.1) .+t),      #grafica la funcion definida M

		#Pedimos que marque el recorrido de laparticula.
		 legend = false, title = "Tiro Parabólico", lw=8, ls=:dot, xlabel = " dis", ylabel =   "Al"  , color = "BLUE") 
	...
	gif(anima, "Tiro_Parabólico.gif", fps=30) #Nos permite tranformar a gif nuestra grafica.
...
begin #Se nos pide tomar las variables que establecimos y tratar de usarlas para cumplir con otra forma de tiro.

	#como ya se establecieron limites y vectores, podemos usar esos mismos y solo ver que podemos modificar en lo que queremos visualizar. 
	
an = @animate for t in 0:0.1:4 #Para cada t en 0:0.1:4
	plot(M.( range(-(5),0, step = 0.1) .+t),      #grafica la funcion M.
		
		 legend = false, title = "Tiro Parabólico 2.0", lw=8, ls=:dot,xlabel = "dis", ylabel = "Al" , xlims=(0,maxn), ylims=(0,may), color = "orange" ) #Colocamos limites en el alcance.
	
		scatter!([Mx(t)],[My(t)])
	end 
	 gif(an, "Tiro_Parabólico.gif", fps=30)
...
ani  = @animate for i in 0:0.1:2π 
plot(h.( range(0,2π, step = 0.1).-i ),      # se grafica la funcion definida h
		 legend = false, title = "Superficie de funcion h",xlabel = "t ", ylabel =  "h(x,y)" ,xlims=(0,25), ylims=(-2,1.5)
			 end
	 gif(ani, "SuperficiedefuncionH.gif", fps=30)
end
