...
function tamanoDePaso(t0, tf, N) #colocamos la funcion y definimos sus parametros.
	print("tamanoDePaso:",(tf-t0)/N)   #colocamos la formula de tamano de paso por el metodo de euler
end

# ╔═╡ c5add344-d78d-4eec-99db-b5bae82767bb
tamanoDePaso(3,7,13) #ya que describimos la funcion, asignamos valores a los parametros.
...
function arregloUniforme(t0, tf, h, N) #definimos la funcion con losparametros que vamosa requerir.

	if N<0  #comensamos un ciclo if para poner las codiciones para N si no es mayor que cero.
		print("Error")
		
	elseif N>0   #cuando N si sea mayor a cero tiene que agregarse un valor a el tamano de paso
	for w∈0:N   #AGREGANDO AL LIMIYE INFERIOR EL VALOR H, donde colocamos un numero delimitante para N. el cual se puede cambiar.
		TAMANODEPASOMASH= t0 + (h)*w #para aumentar el argumento es sumarlo a el limite inicial  del intervalo y este estara multiplicado por w que es un multiplo acendente para el argumento.

		println(TAMANODEPASOMASH)
	end
	end
end

# ╔═╡ 5be2a19f-1930-43f0-ae24-849cde31984f
arregloUniforme(5, 35, 9, 20)
...
begin
function EDO(g, xt0, t) #Definimos a la función con sus parametros.
	    return (xt0 *t)*(h) #Creamos una función sencilla que se pueda utilizar
	end
xt
	g(t) = ( xt0 * t ) * (h)  #Teniendo la función g que debe ser sencilla, podemos definir la condición inicial
	(xt0)= xt0 + h # ti+1 = ti + ih
	xt0=t(xt0)


	e = EDO(g, xt0, t) 
	println(EDO) #Finalmente se imprimen los resultados
end
...
# ╔═╡ 1469a293-2482-4b35-882e-8193d7b2598c
#creoamos un deslizador para los valores de N
 @bind N Slider(-10:0.5:10, default = 1) 

# ╔═╡ ecf4e2e8-821d-4e32-82d8-5eaec50363c3
function paso_euler(ti, xti, g, h) #colocamos la funcion con los parametros.
	xti_1 = xti + h * (g(ti, xti))  #Definimos la formula para poder optener x(ti+1)
	
	for i = 1:N   #colocamos el ciclo for para la sevuencia se N.
	xti =  xti + h * (g(ti, xti))
...
# ╔═╡ 38ec1c38-1a7b-48bb-9092-25920cf3f6ee
paso_euler(9, 16, 9, 78)

# ╔═╡ 7c8c2188-3173-4202-8ba4-83554831d940
function euler(g,xt0, t) #colocamos la funcion con sus parametros.
#tenemos que establecer una condicion inicial.
	    x[1] = xt0
	#el proble nos pide condiciones para i. por lo cual lo tenemos que colectar para que cumpe con las igualdades.
	
		for i = 0:N 
		end 
	
	    for i in 1:N # INICIAMOS un segun for para poder la otra condicion 

	        xt3[i] = xt[i+N] + h * g(t[i+N], xt[i+N]) #sedinimos la ecuacion de euler 
		end #Termina el "for".
	    
	  print(xt3) #Se pide que nos devuelva el resultado x(ti).
	end #Termina la función.




# ╔═╡ 290673af-b421-45bd-bb43-67f412a87d98
euler(cos,6, 17)  #Al mandar valores nos tendrian que salir la respues de una proximacion a xi.

# ╔═╡ c0afe391-1663-49d8-871f-44a176b2b3c0
o=sin

# ╔═╡ 4c48b9e4-f1df-4bb6-ad28-d1ff708c730b
#DESIGNAMOS UNA FUNCION PARA EJECUTAR
begin
	p = range(0, 5, length=100) #colocamos un rango para los esjes 
	Función = [N*(18*10)*50, o *((38)(10))] #damos valores para xt0 y h.
	
end

#El codigo no se efectuo correctamente por que varias variables no esta definido su tipo pero no seri aimpedimento, la idea era que se colocara la funcion ejecutable para poder visualizar una grafica acendente que mostrara las iteracioness.
