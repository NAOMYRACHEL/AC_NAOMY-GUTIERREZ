...
# ╔═╡ 10c2a90e-f792-48a3-980e-6309c9765f45
using Plots
...
# ╔═╡ 97dcdb96-5bdd-47fa-963e-78c01d0a9ce8
rand(Bool,n)

# ╔═╡ e30100d1-95b6-4ae6-827c-7d278638e0e9
rand(Int,n)

# ╔═╡ 3f299c40-b7f2-4dfb-9f49-24007b1c23ff
rand(n)
...
function aleatorioUniforme(a, b) #colocamos la funcion colocando las entradas 
                                 #los limites del intervalo
    F =(b - a)* rand() #agregamos un valor a la diferencia.
   print(F) 
end

# ╔═╡ 7a19cb69-1150-4f32-a77f-a936d5c966d6
aleatorioUniforme(19, 70)
...
function veraleaUniforme(a::Number,b::Number,x)
   entradas = [aleatorioUniforme(a,b) for w = 1:x]
    
    histogram(entradas, bins= 40, title= "verification ")
end

# ╔═╡ 1e333560-9849-4b15-89f7-732faf9b0262

	
	veraleaUniforme(5,8,1000000000) #asignamos valores y consideramos un numero grande para 
...
function unPaso() #colocamos la funcion sin entradas como nos dice el ejercicio.
    if rand(Bool) #Al usar rand bool vamos a tener valores de verdadero y falso por loque ayuda a colocar numeros azarosos. 
		
 return -1 #colocamos los valores que nos pueden salir que nos pide en el problma.
    else
 return 1
    end 
end

# ╔═╡ f3088f2a-abf1-4106-a736-31f17ad990c2
println(unPaso()) #mandamos a imprimir
...
function variosPasos(n) #colocamos la funcion con un parametro.
    arre = rand(Bool, n)#Volvemos a ocupar bool para que nos sigan saliendo numeros alternados de vedadero o falso. 
    arre = 1 .* arre .- 1 #relacioamos los arreglos para que tanto puedan ser 
                          # 1 o menos 1.
    
    return arre
end

# ╔═╡ f92f719a-ab33-4cb9-b3f4-e18ec5dc08e7
print(variosPasos(n))
...
function caminataAleatoria(n) #colocamos de igual forma una 
#colocamos las variables que ocupa la condicion inical  condicion inicial que nos describen, que son el numero de pasos y la partida.
	
    paso = variosPasos(n)
    partida = cumsum(paso)
      posin= [0; paso] #escribimos la condicion inicial. 
    return posin
end

# ╔═╡ a68af82b-33f4-4eba-8343-30f95bdb5326
println(caminataAleatoria(n))
...
begin
function graficaCaminata(posin) #definimos la funcion.
    n = length(posin)     #colocamos el valor de en dependiente de c, y como es el paso.
    p = 0:n-1
    scatter(p, posin, title = "Caminata", xlabel = "Num. de Pasos", ylabel = "Posición") #Utilizamos scatter por tiempo discreto
end
end

# ╔═╡ 61f48034-475d-4ef3-a4ac-45328149d405
graficaCaminata(posin)  #aqui el codigo no de ejecuto bien por que dice que no esta definido posin pero este ya se definio en celdas anteriores. 

#la idea era poder visualizar

# ╔═╡ b835824f-8b7d-4c48-a508-6d3081ee6941
n2=65
...
function graficaCaminata!(posin)
    n = length(posin) #igual que en la aterior definimos de la funcion y le damos valores a las variables 
    p = 0:n-1
    plot(p, posin, title= "Caminata!", linestyle = :dot, marker = :circle,color = "green",  markersize = 3, label = "Caminata Al.", xlabel = "Num. de Pasos", ylabel = "Posición") #
end

# ╔═╡ c396feab-637c-45a9-82b9-9d59398e452d
begin
	   #al asignar valores se imprime y considera a n2.
	for i in 1:12
	    posin = caminataAleatoria(n2)
	    graficaCaminata!(posin)
	end
	plot!(legend)
end

#para que imprima la grafica falta definir legend, pero prove con un valor y siguio mandando error.
